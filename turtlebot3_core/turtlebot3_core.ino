...
void setup()
{
  ...
  nh.advertiseService(gpio_server); //by kccistc
...
void gpioCallback(const bot3gpio::Request & req, bot3gpio::Response & res){
  if (req.a == 3) {  // 새로운 명령 추가: 2번은 분무기 활성화
    digitalWrite(SPRAYER_PIN, LOW);  // LOW 신호로 분무기 작동
    delay(3000);  // 1초 동안 분무 (필요에 따라 조정)
    digitalWrite(SPRAYER_PIN, HIGH);  // 다시 비활성화
    res.result = 1;
  } else if(req.a==2) {
    digitalWrite(SPRAYER_PIN, LOW);  // LOW 신호로 분무기 작동
    delay(1000);
